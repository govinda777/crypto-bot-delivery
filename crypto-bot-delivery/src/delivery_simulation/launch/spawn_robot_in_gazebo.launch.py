...
def generate_launch_description():
    ...
    robot_urdf_path = os.path.join(
        pkg_delivery_simulation, "models", "delivery_bot.urdf.xacro"
    )
    ...
    spawn_entity_node = Node(
        ...
        arguments=[
            "-topic",
            "robot_description",  # Use the robot_description topic
            "-entity",
            "delivery_bot",  # Name of the entity in Gazebo
            "-x",
            "0.0",  # Initial X pose
            "-y",
            "0.0",  # Initial Y pose
            "-z",
            "0.1",  # Initial Z pose (slightly above ground)
            "-Y",
            "0.0",  # Initial Yaw
        ],
