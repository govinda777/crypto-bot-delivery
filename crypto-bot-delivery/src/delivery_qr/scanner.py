    ...
    try:
        ...
        if decoded_objects:
            ...
            try:
                ...
                print(
                    f"Error decoding JSON from QR data: {e}. Data was: '{qr_data_str}'"
                )
                return None
        else:
...
if __name__ == "__main__":
    ...
    if not (os.path.exists(pickup_qr_path) and os.path.exists(delivery_qr_path)):
        ...
        try:
            ...
            print(
                "Could not import 'generate_qr_code' for self-contained example. "
        ...
        if generate_qr_code:  # Proceed if import was successful
            sample_data_pickup = {
                "order_id": "scan_test_pickup_123",
                "type": "pickup",
                "timestamp": time.time(),
                "token": "scan_token_pickup_xyz",
            }
            if generate_qr_code(sample_data_pickup, pickup_qr_path):
                print(f"Generated sample: {pickup_qr_path}")

            sample_data_delivery = {
                "order_id": "scan_test_delivery_456",
                "type": "delivery",
                "timestamp": time.time(),
                "token": "scan_token_delivery_abc",
            }
    ...
    if os.path.exists(pickup_qr_path):
        ...
        print(
            f"Pickup QR file not found at {pickup_qr_path}. Run generator.py example first or ensure path is correct."
        )
    ...
    if os.path.exists(delivery_qr_path):
        ...
        if delivery_data:
            ...
            assert (
                delivery_data.get("type") == "delivery"
            )  # Basic assertion for example
        else:
            print(f"Failed to decode delivery QR from {delivery_qr_path}")
    else:
        print(
            f"Delivery QR file not found at {delivery_qr_path}. Run generator.py example first or ensure path is correct."
        )

    print("\nAttempting to scan a non-existent QR file:")
    scan_qr_code_from_file(
        non_existent_path
    )  # Expected to fail and print "File not found" error
