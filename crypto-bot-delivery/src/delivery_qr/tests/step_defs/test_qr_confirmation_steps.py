...
@given(
    parsers.parse('a unique order ID "{order_id}"'), target_fixture="order_id_fixture"
)
...
@when(
    parsers.parse(
        'a QR code is generated for "{order_id}" with type "{conf_type}" and token "{token}"'
    )
)
def when_qr_code_generated(
    scenario_context, order_id_fixture, conf_type, token
):  # Use order_id_fixture
    data_to_encode = {
...
def when_generated_qr_scanned(scenario_context, qr_type):
    ...
    assert (
        scenario_context.get("generated_qr_type") == qr_type
    ), "QR type mismatch in test step"
...
@then(
    parsers.parse(
        'the decoded QR data should contain "{expected_order_id}", type "{expected_type}", and token "{expected_token}"'
    )
)
def then_decoded_data_matches(
    scenario_context, expected_order_id, expected_type, expected_token
):
    ...
    assert isinstance(
        decoded_data["timestamp"], (float, int)
...
@then(
    parsers.parse(
        'the system expects token "{expected_token}" for "{order_id}" type "{conf_type}"'
    )
)
def then_system_expects_token(scenario_context, expected_token, order_id, conf_type):
    ...
    assert (
        scenario_context["order_id_fixture"] == order_id
    ), "Order ID mismatch in expectation step"
    assert (
        scenario_context["confirmation_type"] == conf_type
    ), "Confirmation type mismatch in expectation step"
...
def then_validation_fails_token_mismatch(scenario_context):
    ...
    assert (
        expected_system_token is not None
    ), "Expected system token was not set in test context"
    assert (
        decoded_token != expected_system_token
...
def then_scanning_fails_no_data(scenario_context):
    ...
    ), "QR scanning was expected to fail or return None, but it returned data."
