...
ETHEREUM_NODE_URL = os.environ.get("ETH_NODE_URL", "http://127.0.0.1:7545") # Default to Ganache
CONTRACT_ABI_PATH = os.environ.get("CONTRACT_ABI_PATH", "crypto-bot-delivery/smart_contracts/Escrow.json") # Adjusted path
# Placeholder: Replace with actual deployed contract address
DEPLOYED_CONTRACT_ADDRESS = os.environ.get("ESCROW_CONTRACT_ADDRESS", "0x0000000000000000000000000000000000000000")
...
CLIENT_PRIVATE_KEY = os.environ.get("CLIENT_PRIVATE_KEY", "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa") # Replace with a real private key from Ganache
ROBOT_PRIVATE_KEY = os.environ.get("ROBOT_PRIVATE_KEY", "0xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb")   # Replace with another real private key from Ganache
...
def load_contract(w3: Web3, abi_path: str, contract_address: str):
    ...
    try:
        ...
        corrected_abi_path = os.path.join(os.path.dirname(__file__), '..', '..', abi_path)
        ...
        if not w3.is_address(contract_address):
            print(f"Invalid contract address: {contract_address}. Not a valid Ethereum address.")
            return None
        ...
        print(f"ABI file not found at {corrected_abi_path}. Make sure 'Escrow.json' exists.")
        print(f"Current working directory: {os.getcwd()}")
        print(f"Attempted ABI path: {corrected_abi_path}")
        # Try listing files in smart_contracts to help debug
        smart_contracts_dir = os.path.join(os.path.dirname(__file__), '..', '..', 'crypto-bot-delivery', 'smart_contracts')
        try:
            print(f"Contents of {smart_contracts_dir}: {os.listdir(smart_contracts_dir)}")
        except FileNotFoundError:
        ...
        try:
            with open(CONTRACT_ABI_PATH, 'r') as f:
                pass # File exists
            print(f"ABI file found at raw CONTRACT_ABI_PATH: {CONTRACT_ABI_PATH}, but path correction might be failing.")
        except FileNotFoundError:
            print(f"ABI file also not found at raw CONTRACT_ABI_PATH: {CONTRACT_ABI_PATH}")
...
def build_and_send_tx(w3: Web3, contract_function, sender_account, gas_estimate_multiplier=1.2):
    ...
    try:
        ...
        # Estimate gas
        estimated_gas = contract_function.estimate_gas({'from': sender_account.address, 'value': tx_params.get('value', 0)})
        ...
        if hasattr(e, 'message') and "revert" in e.message:
            print(f"Contract execution reverted. Reason might be in message: {e.message}")
        return None

def create_delivery_order(w3: Web3, contract, order_id: str, 
                          delivery_provider_address: str, amount_wei: int, 
                          pickup_secret: str, delivery_secret: str, 
                          client_account):
    """Creates a new delivery order on the smart contract."""
    try:
        order_id_bytes32 = Web3.to_bytes(text=order_id).ljust(32, b'\0') # Ensure 32 bytes
        pickup_hash = generate_secret_hash(w3, pickup_secret)
        delivery_hash = generate_secret_hash(w3, delivery_secret)

        print(f"Creating order '{order_id}' for {delivery_provider_address} with amount {amount_wei} wei.")
        ...
        transaction = contract_function.build_transaction(tx_params)
        signed_tx = w3.eth.account.sign_transaction(transaction, private_key=client_account.key)
        tx_hash = w3.eth.send_raw_transaction(signed_tx.rawTransaction)
        ...
        if tx_receipt.status == 1:
            print(f"Order '{order_id}' created successfully. Block: {tx_receipt.blockNumber}")
            return tx_receipt
        else:
...
def confirm_pickup(w3: Web3, contract, order_id: str, pickup_secret: str, delivery_provider_account):
    """Confirms pickup of an order."""
    try:
        order_id_bytes32 = Web3.to_bytes(text=order_id).ljust(32, b'\0')
        print(f"Confirming pickup for order '{order_id}' with secret '{pickup_secret}' by {delivery_provider_account.address}")
        
        contract_function = contract.functions.confirmPickup(order_id_bytes32, pickup_secret)
...
def confirm_delivery_and_release(w3: Web3, contract, order_id: str, delivery_secret: str, client_account):
    """Confirms delivery of an order and releases funds."""
    try:
        order_id_bytes32 = Web3.to_bytes(text=order_id).ljust(32, b'\0')
        print(f"Confirming delivery for order '{order_id}' with secret '{delivery_secret}' by {client_account.address}")

        contract_function = contract.functions.confirmDelivery(order_id_bytes32, delivery_secret)
...
def get_order_details(w3: Web3, contract, order_id: str):
    ...
    try:
        ...
        status_map = {0: "Created", 1: "PickedUp", 2: "Delivered", 3: "Cancelled", 4: "Refunded"}
...
if __name__ == '__main__':
    ...
    abi_full_path = os.path.join(os.path.dirname(__file__), '..', '..', CONTRACT_ABI_PATH)
    if not os.path.exists(abi_full_path):
        print(f"\nERROR: ABI file not found at {abi_full_path}")
        print("Please ensure 'Escrow.json' exists in 'crypto-bot-delivery/smart_contracts/'.")
        print("You might need to compile your Solidity contract and place the ABI JSON there.")
    ...
    if DEPLOYED_CONTRACT_ADDRESS == "0x0000000000000000000000000000000000000000":
        ...
        print("Please deploy your Escrow.sol contract and update this address in the script or via environment variable.")
    ...
    if not escrow_contract:
        print("Exiting: Could not load Escrow contract. Ensure ABI is correct and contract is deployed at the address.")
        exit()
    ...
    create_tx_receipt = create_delivery_order(
        w3, escrow_contract, order_id, 
        robot_account.address, amount_wei,
        pickup_secret_phrase, delivery_secret_phrase,
        client_account
    )
    ...
    print(f"Initial Client Balance: {w3.from_wei(initial_client_balance, 'ether')} ETH")
    print(f"Final Client Balance:   {w3.from_wei(final_client_balance, 'ether')} ETH (includes gas costs)")
    
    print(f"Initial Robot Balance: {w3.from_wei(initial_robot_balance, 'ether')} ETH")
    print(f"Final Robot Balance:   {w3.from_wei(final_robot_balance, 'ether')} ETH (received payment, paid gas for pickup)")
