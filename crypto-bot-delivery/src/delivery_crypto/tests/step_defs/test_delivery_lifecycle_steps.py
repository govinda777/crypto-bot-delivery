...
def web3_connection():
    ...
    if not w3.eth.accounts:
        pytest.skip(
            "No accounts found in Ganache. Ensure Ganache is running and has accounts."
        )
...
def escrow_contract_abi():
    if not os.path.exists(CONTRACT_ABI_PATH):
        pytest.skip(
            f"Contract ABI not found at {CONTRACT_ABI_PATH}. Run compilation first."
        )
...
def escrow_contract_bytecode():
    if not os.path.exists(CONTRACT_BYTECODE_PATH):
        pytest.skip(
            f"Contract bytecode not found at {CONTRACT_BYTECODE_PATH}. Run compilation first (ensure .bin output)."
        )
...
@pytest.fixture(
    scope="function"
)  # Re-deploy for each test function to ensure isolation
def escrow_contract(web3_connection, escrow_contract_abi, escrow_contract_bytecode):
    ...
    try:
        tx_hash = Escrow.constructor().transact(
            {"from": deployer_account, "gas": GAS_LIMIT}
        )
    ...
    if not tx_receipt or not tx_receipt.contractAddress:
        pytest.fail(
            "Contract deployment failed: No contract address in receipt or receipt is None."
        )

    contract_instance = w3.eth.contract(
        address=tx_receipt.contractAddress, abi=escrow_contract_abi
    )
...
def accounts(web3_connection):
    ...
    if len(w3.eth.accounts) < 3:
        pytest.skip(
            "Not enough accounts in Ganache for testing (need at least 3). Skipping."
        )
...
def given_client_account(accounts, account_name, web3_connection, scenario_context):
    assert (
        account_name in accounts
    ), f"Account '{account_name}' not found in fixture. Available: {list(accounts.keys())}"
    ...
    assert balance > Web3.to_wei(
        0.5, "ether"
    ), f"{account_name} ({address}) does not have sufficient funds (has {Web3.from_wei(balance, 'ether')} ETH)."
...
@given(
    parsers.parse(
        '{user_type:S} wants to create a delivery order with ID "{order_id}" for {provider_type:S}'
    )
)
def given_user_wants_to_create_order(
    scenario_context, user_type, order_id, provider_type
):
    ...
    if "users" not in scenario_context or user_type not in scenario_context["users"]:
        pytest.fail(
            f"User '{user_type}' not found in scenario_context. Ensure a prior step defines this user."
        )
    if (
        "users" not in scenario_context
        or provider_type not in scenario_context["users"]
    ):
        pytest.fail(
            f"Provider '{provider_type}' not found in scenario_context. Ensure a prior step defines this provider."
        )
...
def given_order_amount(scenario_context, amount_eth):
    ...
    scenario_context["current_order_amount_wei"] = Web3.to_wei(
        Decimal(amount_eth), "ether"
    )
...
def when_creates_order(
    web3_connection, escrow_contract, scenario_context, accounts, user_role, order_id
):
    w3 = web3_connection
    ...
    if user_role != client_name:
        pytest.fail(
            f"Step user '{user_role}' does not match context client '{client_name}' for creating order '{order_id}'."
        )

    client_account = scenario_context["users"][client_name]
    robot_account_address = scenario_context["users"][
        scenario_context["current_order_provider_name"]
    ]

    if order_id != scenario_context["current_order_id"]:
        pytest.fail(
            f"Order ID in step '{order_id}' does not match context '{scenario_context['current_order_id']}'."
        )
    ...
    scenario_context[f"initial_contract_balance_for_{order_id}"] = get_eth_balance(
        w3, escrow_contract.address
    )
    scenario_context[f"initial_client_balance_for_{order_id}"] = get_eth_balance(
        w3, client_account
    )
    ...
    try:
        ...
        if receipt.status == 0:  # Store explicit error if transaction reverted
            scenario_context[f"last_tx_error_for_{order_id}"] = (
...
@when(
    parsers.parse(
        '{user_role} confirms pickup for order "{order_id}" with secret "{secret}"'
    )
)
def when_confirms_pickup(
    web3_connection,
    escrow_contract,
    scenario_context,
    accounts,
    ...
    try:
        tx_hash = escrow_contract.functions.confirmPickup(
            order_id_bytes32, secret
        ).transact({"from": actor_account, "gas": GAS_LIMIT})
        receipt = w3.eth.wait_for_transaction_receipt(tx_hash, timeout=120)
        scenario_context[f"last_tx_receipt_for_{order_id}"] = receipt
        scenario_context[f"last_tx_hash_for_{order_id}"] = tx_hash
        if receipt.status == 0:
...
@when(
    parsers.parse(
        '{user_role} confirms delivery for order "{order_id}" with secret "{secret}"'
    )
)
def when_confirms_delivery(
    web3_connection,
    escrow_contract,
    scenario_context,
    accounts,
    ...
    provider_name_key = scenario_context.get(
        "current_order_provider_name", "RoboDeliverer"
    )  # Default if not set by specific order context
    if order_id != scenario_context.get(
        "current_order_id"
    ):  # If current_order_id is not this order, this is problematic
    ...
    scenario_context[
        f"{provider_name_key.lower()}_balance_before_delivery_for_{order_id}"
    ] = get_eth_balance(w3, robot_account_address)
    scenario_context[f"contract_balance_before_delivery_for_{order_id}"] = (
        get_eth_balance(w3, escrow_contract.address)
    )
    scenario_context[
        f"{user_role.lower()}_balance_before_confirm_delivery_for_{order_id}"
    ] = get_eth_balance(w3, actor_account)

    try:
        tx_hash = escrow_contract.functions.confirmDelivery(
            order_id_bytes32, secret
        ).transact({"from": actor_account, "gas": GAS_LIMIT})
        receipt = w3.eth.wait_for_transaction_receipt(tx_hash, timeout=120)
        scenario_context[f"last_tx_receipt_for_{order_id}"] = receipt
...
def when_cancels_order(
    ...
    scenario_context[f"{user_role.lower()}_balance_before_cancel_for_{order_id}"] = (
        get_eth_balance(w3, actor_account)
    )
    scenario_context[f"contract_balance_before_cancel_for_{order_id}"] = (
        get_eth_balance(w3, escrow_contract.address)
    )
...
@then(
    parsers.parse(
        'the order "{order_id}" should be successfully created on the blockchain'
    )
)
def then_order_successfully_created(
    escrow_contract, scenario_context, order_id, accounts
):
    if f"last_tx_error_for_{order_id}" in scenario_context:
        pytest.fail(
            f"Transaction for creating order '{order_id}' failed with: {scenario_context[f'last_tx_error_for_{order_id}']}"
        )

    receipt = scenario_context.get(f"last_tx_receipt_for_{order_id}")
    assert (
        receipt is not None
    ), f"Transaction receipt not found in context for order '{order_id}'"
    assert (
        receipt.status == 1
    ), f"Transaction to create order '{order_id}' failed (receipt status 0)"

    order_id_bytes32 = order_id.encode("utf-8").ljust(32, b"\0")
    order_details = escrow_contract.functions.orders(order_id_bytes32).call()

    client_name = scenario_context[
        "current_order_client_name"
    ]  # Set by Given step for this order
    provider_name = scenario_context[
        "current_order_provider_name"
    ]  # Set by Given step for this order

    assert (
        order_details[0] == accounts[client_name]
    ), f"Order client mismatch for '{order_id}'."
    assert (
        order_details[1] == accounts[provider_name]
    ), f"Order provider mismatch for '{order_id}'."
    assert (
        order_details[2] == scenario_context["current_order_amount_wei"]
    ), f"Order amount mismatch for '{order_id}'."
    assert (
        order_details[3] == 0
    ), f"Order status mismatch for '{order_id}'. Expected 0 (Created)."


@then(parsers.parse('the contract balance should increase by "{amount_eth}" ETH'))
def then_contract_balance_increases(
    web3_connection, escrow_contract, scenario_context, amount_eth
):
    w3 = web3_connection
    ...
    initial_contract_balance = scenario_context[
        f"initial_contract_balance_for_{order_id}"
    ]

    actual_increase = current_contract_balance - initial_contract_balance
    assert (
        actual_increase == expected_increase_wei
    ), f"Contract balance increase mismatch for order '{order_id}'. Expected {expected_increase_wei}, got {actual_increase}"


@then(
    parsers.parse('the status of order "{order_id}" should be "{expected_status_str}"')
)
def then_order_status_is(escrow_contract, order_id, expected_status_str):
    ...
    for (
        ...
        status_map.items()
    ):  # Find string name for actual enum value for better error messages
        if v == actual_status_enum:
            actual_status_str = k
            break

    assert (
        actual_status_enum == expected_status_enum
...
def then_pickup_confirmed(scenario_context, order_id):
    if f"last_tx_error_for_{order_id}" in scenario_context:
        pytest.fail(
            f"Transaction for confirming pickup for order '{order_id}' failed with: {scenario_context[f'last_tx_error_for_{order_id}']}"
        )
    receipt = scenario_context.get(f"last_tx_receipt_for_{order_id}")
    assert (
        receipt is not None
    ), f"Transaction receipt not found for pickup confirmation of order '{order_id}'"
    assert (
        receipt.status == 1
    ), f"Pickup confirmation transaction for order '{order_id}' failed (receipt status 0)"
...
def then_delivery_confirmed(scenario_context, order_id):
    if f"last_tx_error_for_{order_id}" in scenario_context:
        pytest.fail(
            f"Transaction for confirming delivery for order '{order_id}' failed with: {scenario_context[f'last_tx_error_for_{order_id}']}"
        )
    receipt = scenario_context.get(f"last_tx_receipt_for_{order_id}")
    assert (
        receipt is not None
    ), f"Transaction receipt not found for delivery confirmation of order '{order_id}'"
    assert (
        receipt.status == 1
    ), f"Delivery confirmation transaction for order '{order_id}' failed (receipt status 0)"


@then(
    parsers.parse(
        "{user_role}'s balance should increase by approximately '{amount_eth}' ETH (minus gas)"
    )
)
def then_balance_increases_approx(
    web3_connection, scenario_context, user_role, amount_eth, accounts
):
    w3 = web3_connection
    order_id = scenario_context["current_order_id"]
    ...
    if balance_before_key not in scenario_context:
        pytest.fail(
            f"Balance before key '{balance_before_key}' not found in scenario_context for user '{user_role}' and order '{order_id}'."
        )
    ...
    assert (
        actual_increase_wei == expected_increase_wei
...
def then_contract_balance_for_order_is_zero(
    web3_connection, escrow_contract, scenario_context, order_id
):
    w3 = web3_connection
    ...
    if f"contract_balance_before_delivery_for_{order_id}" in scenario_context:
        initial_relevant_balance = scenario_context[
            f"contract_balance_before_delivery_for_{order_id}"
        ]
        expected_change_wei = -order_amount_wei
    elif f"contract_balance_before_cancel_for_{order_id}" in scenario_context:
        initial_relevant_balance = scenario_context[
            f"contract_balance_before_cancel_for_{order_id}"
        ]
        expected_change_wei = -order_amount_wei
    else:
        pytest.fail(
            f"Contract balance context (before delivery/cancel for order {order_id}) not properly set up."
        )

    actual_change_wei = contract_balance_after_action - initial_relevant_balance
    assert (
        actual_change_wei == expected_change_wei
    ), f"Contract balance change mismatch for order '{order_id}'. Expected change {expected_change_wei}, got {actual_change_wei}"


@then(
    parsers.parse('the order "{order_id}" should be marked as "{expected_status_str}"')
)
def then_order_marked_as_status(
    escrow_contract, scenario_context, order_id, expected_status_str
):
    if expected_status_str not in ["FailedStateOrInvalid"]:
        if f"last_tx_error_for_{order_id}" in scenario_context:
            pytest.fail(
                f"Transaction for order '{order_id}' to reach status '{expected_status_str}' failed unexpectedly with: {scenario_context[f'last_tx_error_for_{order_id}']}"
            )
        receipt = scenario_context.get(f"last_tx_receipt_for_{order_id}")
        assert (
            receipt is not None
        ), f"Receipt for order '{order_id}' not found when checking for status '{expected_status_str}'."
        assert (
            receipt.status == 1
        ), f"Transaction for order '{order_id}' failed (status 0) when expecting status '{expected_status_str}'."

    then_order_status_is(
        escrow_contract, order_id, expected_status_str
    )  # Re-use the status checking logic


@then(
    parsers.parse(
        "{user_role}'s balance should be refunded approximately '{amount_eth}' ETH (minus gas)"
    )
)
def then_balance_refunded_approx(
    web3_connection, scenario_context, user_role, amount_eth, accounts
):
    w3 = web3_connection
    order_id = scenario_context["current_order_id"]
    ...
    balance_before_cancel = scenario_context[
        f"{user_role.lower()}_balance_before_cancel_for_{order_id}"
    ]
    ...
    assert (
        actual_change_wei == expected_net_change_wei
